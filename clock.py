...
def run(frame):
    ...
    symbol_label = frame.new_label('CO₂ H₂O CH₄  é å Î ç Ç', 'cclock-10', cv)
    f = 0
    while True:
        text = ccui.format_deadline_module(carbon_module)
        carbon_label = frame.new_label(text, 'cclock-10', cv)
        f += 1
        count_label = frame.new_label('%08d' % f, 'cclock-10', cv)
